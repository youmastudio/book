...
class WebSocketServer extends EventEmitter {
    ...
    close(cb) {
        ...
        if (this.clients.clients) {
            for (const client of this.clients.clients) client.terminate();
        }
    ...
    handleUpgrade(req, socket, head, cb) {
        ...
        if (
            !this.shouldHandle(req) || !req.headers.upgrade ||
            req.headers.upgrade.toLowerCase() !== 'websocket' || !req.headers['sec-websocket-key'] ||
...
function Clients() {
    ...
    this.initClients = new Map();
}
Clients.prototype = {
    add: function (ws) {
        this.clients.add(ws)
        if (ws.clientId) {
            initClients.set(ws.clientId, ws)
        }
    },
    delete: function (ws) {
        this.clients.delete(ws)
        if (ws.clientId) {
            initClients.delete(ws.clientId)
        }
    },
    setId: function (id, ws) {
        initClients.set(id, ws)
    }
}
